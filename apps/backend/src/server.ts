...
// Custom CORS middleware to handle bookmarklet routes specially
app.use((req, res, next) => {
  const origin = req.get('origin');
  const isBookmarkletPath = req.path.includes('/api/wishlist/bookmarklet');

  if (isBookmarkletPath) {
    // Allow any HTTPS origin for bookmarklet endpoints
    if (origin) {
  ...
  // For regular requests, use standard CORS
